defmodule SddOrchestrator.RepositorySelection.Stub do
  ...
  defp result(request) do
    case AttachmentCodec.encode_request(request) do
      {:ok, payload} -> answered(request.id, folder(), payload)
  ...
  defp answered(request_id, path, payload) do
    with {:ok, _roots} <- RepositoryValidation.root_commit_ids(path),
         {:ok, identity} <- generated_identity(payload["generate"], path) do
      selected(request_id, path, payload["candidates"], identity)
    else
      {:error, reason} -> refused(request_id, reason)
    end
  end

  defp selected(request_id, path, candidates, identity) do
    payload = %{
      "request_id" => request_id,
      "outcome" => "selected",
      "folder_name" => Path.basename(path),
      "matches" => matching_refs(candidates, path)
    }
