defmodule SddOrchestrator.Privacy.ParticipationSupportElevation do
  @moduledoc """
  One verified, purpose-bound, time-bounded, least-privilege support-elevation
  grant (specs/26 Task 3, AC-03).

  Mirrors `SddOrchestrator.Privacy.DeliverySupportElevation` (specs/18 Task 2):
  exceptional support access to participation data is separate from the
  approved participation and operations access boundaries
  (`SddOrchestrator.Participation.Boundary`,
  `SddOrchestrator.Privacy.ParticipationOperationsAccess`) and disabled by
  default. A grant is scoped to exactly one project's incident, carries one
  purpose from a fixed, closed vocabulary, and defaults to `scope: :metadata`.
  A caller must explicitly elevate `scope` to `:content` for a grant to ever
  authorize a content read — see
  `SddOrchestrator.Privacy.ParticipationSupportAccess.authorize_content_read/2`.
  ...
  Nothing here stores participation content: no invitation email, participant
  display name, project name, invitation credential, or free-text content. It
  names only an operations account, a project, a purpose atom, a scope atom,
  and the grant's own lifecycle timestamps.
  """
  use Ecto.Schema
  ...
  schema "participation_support_elevations" do
  ...
  def issue_changeset(%__MODULE__{} = elevation, attrs) do
    ...
    |> check_constraint(:purpose, name: :participation_support_elevations_purpose_allowed)
    |> check_constraint(:scope, name: :participation_support_elevations_scope_allowed)
    |> check_constraint(:expires_at, name: :participation_support_elevations_bounded_expiry)
    |> check_constraint(:revoked_at, name: :participation_support_elevations_revocation_pairing)
