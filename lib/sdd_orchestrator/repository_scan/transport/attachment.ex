defmodule SddOrchestrator.RepositoryScan.Transport.Attachment do
  @moduledoc """
  Hands one scan request to the worker attached for its Mac.
  ...
  the device workspace the gateway credential named. A scan travels there and
  not on the project-scoped topic for the same reason a metadata read does:
  the folder it scans is held by the Mac's own worker session, under the
  selection reference the binding was verified with.

  The request is aimed at one named worker inside that workspace, never at
  whoever happens to be attached. A person picked a machine, the binding was
  verified on that machine, and the scan has to run on the same one.
  ...
  means a worker is attached but did not negotiate the scan vocabulary, which
  is a worker too old to ask. `:transport_error` means the request itself
  could not be encoded, which is a defect on this side and never the caller's
  fault.
  ...
  Nothing pushed from here holds an absolute path, a remote URL, or file
  content. The payload is built by
  `SddOrchestrator.RepositoryScan.AttachmentCodec`, which is closed to the
  four fields a request is made of.
  """
  ...
  @doc "The capability a worker declares at attach to be askable for a repository scan."
  @spec capability() :: String.t()
  def capability, do: @capability

  @impl true
  def push(%ScanRequest{} = request) do
    with {:ok, channel} <- capable_attachment(request),
         {:ok, payload} <- encode(request) do
      send(channel, {:repository_scan, payload})
      {:ok, channel}
    end
  end

  @impl true
  def cancel(%ScanRequest{} = request) do
    with {:ok, channel} <- capable_attachment(request),
         {:ok, payload} <- AttachmentCodec.encode_cancellation(request) do
      send(channel, {:repository_scan_cancel, payload})
    end
