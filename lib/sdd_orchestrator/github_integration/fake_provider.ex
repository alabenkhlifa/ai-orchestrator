# The deterministic GitHub stand-in lives in `lib/` behind the same compile-time
# gate as the browser-harness bootstrap, so one fake serves both the ExUnit suite
# and the isolated end-to-end server instead of a second copy being written for
# the browser. The gate is set in `config/test.exs` and, in development, only
# under `E2E_MODE`; no production configuration sets it, so a production build
# contains no such module and there is no provider stand-in to reach or re-enable.
