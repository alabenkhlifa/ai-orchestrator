defmodule SddOrchestrator.Portability.HostedRestore do
  ...
  def restore(
    ...
    with %PersonalWorkspace{} <- Repo.get(PersonalWorkspace, authority.id),
         ...
         {:ok, specification_values} <- specification_values(package),
         :ok <- decision_matches_package(decision, package),
         {:ok, multi} <-
           restore_multi(
             authority,
             package,
  ...
  defp decision_matches_package(
         ...
         %ProjectPackage{
           ...
           repository: %PackageSection{
             content: %{
               "provider" => packaged_provider,
               "repository_id" => packaged_repository_id
             }
           }
         }
       ) do
  ...
  defp decision_matches_package(_decision, _package), do: {:error, :invalid_restore}

  defp specification_values(%ProjectPackage{
  ...
  defp specification_values(_package), do: {:error, :invalid_restore}
