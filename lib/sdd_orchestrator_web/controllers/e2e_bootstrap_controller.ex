defmodule SddOrchestratorWeb.E2EBootstrapController do
  ...
  application session is only issued by a live GitHub round trip, and a hosted
  session is only issued by a delivered passwordless credential. Neither is
  available to the local end-to-end server, which is why the authenticated
  participation and feature-delivery matrices were previously unprovable in a
  real browser.
  ...
  lifecycle transition table — so every seeded state is a state the product can
  actually produce, and the delivered invitation credential is read back out of
  the delivered message rather than reconstructed here.

  ## Why this cannot reach production

  The route is excluded by construction, not by a runtime check. The router
  wraps it in `Application.compile_env(:sdd_orchestrator, :e2e_bootstrap,
  false)`; no production configuration sets that key, so the route is not
  compiled into a production build and there is nothing to reach. `create/2`
  re-checks the same flag at runtime and answers `404` when it is off, which
  keeps a build whose configuration is later changed from serving it.
  """
  ...
  defp run(conn, "project_member", params) do
    %{project: project, owner: owner, participant: participant, pending: pending} = member_graph()
