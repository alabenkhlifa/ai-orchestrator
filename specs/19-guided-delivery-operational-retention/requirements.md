...
- Thirty-day cleanup of inactive command payloads, checkpoints, superseded artifacts, terminal preview deployments whose remote counterpart is confirmed released, and spent attempt-lease claims.
...
- A lease is owner and expiry together. They are cleared as a pair, never one without the other, because a claim with only one half would read as current forever.
- The fence token is retained with its attempt. It is a positive, unique, structural ordering token rather than a description of anyone, and it expires when the attempt row itself is deleted under the project's own lifecycle.
...
- [AC-09] Given a run attempt is terminal and reaches 30 days, when retention runs, then its lease owner and lease expiry are cleared together while the attempt row, its fence token, and its participant-visible outcome remain intact.
