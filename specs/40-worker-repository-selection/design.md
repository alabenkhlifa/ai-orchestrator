...
- The worker's `Worker.RepositorySelection` holds the one pending request, exposes it to the app, and accepts the app's answer. Once the app hands it a path or a cancellation, it runs the Git check, generates a new identity when asked, matches the folder against every candidate identity it was given, takes the folder name, and sends the result. It compares a candidate the same way the control plane would: a portable identity by digest, a legacy one against its own configured workspace salt. Anything less would quietly weaken the duplicate rule this slice promises to leave intact. The path is used inside that process and never written, logged, or sent.
...
- Candidate identities are repository identities, never paths or names. They are sent only to a worker paired to the requesting workspace. A candidate may be a portable identity (salt and digest) or a legacy workspace-salted digest, and the worker tells them apart by parsing the value itself, exactly as `Devices.matches_repository?/3` does on the control plane. A legacy comparison needs the workspace salt, which is the device workspace id the worker already holds in its own configuration, so no salt travels in a request. The result carries the folder name (the last path segment) and identities. No path, remote URL, Git history, file name, or content is placed in a request, a result, a log, a diagnostic, or an analytics event on either side.
