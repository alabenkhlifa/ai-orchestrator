...
The worker app has a native picker (`WorkspaceFolderPicking`, implemented with `NSOpenPanel` on the main thread), but only the Swift side ever calls the Elixir side, through `bin/worker rpc`. The app polls the release for connection status on a timer. No path from Elixir to Swift exists.
...
- The request rides the Mac-scoped attachment from `specs/39`, not the project-scoped `worker:` topic, because a worker being asked to pick a repository has, by definition, no project binding yet. The attachment gains one message pair, `repository_selection` out and `repository_selection_result` in, with its own small codec and a capability flag the worker declares at attach time. `Delivery.WorkerProtocol` and `ProtocolCodec` are untouched; run commands and picker requests are different things and stay in different codecs.
...
- The app learns about a pending request the same way it learns everything else from the release: its existing status poll asks, over `bin/worker rpc`, whether a selection is pending, shows `NSOpenPanel` on the main thread when one is, and answers with one more rpc call. The poll runs every two seconds while the worker is attached. Inventing a push channel from Elixir to Swift would be cleaner and is not worth its own transport for a request a person answers in tens of seconds.
...
- Worker app (`native/worker-app`): pending-selection poll, `NSOpenPanel` presentation, answer and cancel rpc calls.
...
- The worker uses the path only inside the answering process. The app does not retain the chosen path after answering.
...
- Attachment message `repository_selection` (control plane to worker): `request_id`, `candidates` (a list of `ref` plus identity), `generate` (boolean), `expires_at`. Attachment message `repository_selection_result` (worker to control plane): `request_id`, `outcome`, `folder_name`, `matches` (list of `ref`), `identity` (or absent). A worker declares the `repository_selection` capability on attach; a request to a worker without it is refused with `:worker_needs_update`.
...
- Worker rpc surface for the app: `Worker.RepositorySelection.pending/0` answers the pending request id or `nil`; `answer/2` takes the request id and a path or `:cancelled`.
...
### The app polls for a pending request

- Choice: The app's existing rpc status poll also asks for a pending selection, at a two-second interval while attached, instead of a new Elixir-to-Swift channel.
- Reason: Every existing crossing is Swift to Elixir over `bin/worker rpc`, and a person answering a folder picker will not notice up to two seconds of delay.
- Consequence: A polling cost while attached, and a small delay before the panel appears. If a later feature needs the app to react faster, a push channel replaces this poll without changing the request contract.
