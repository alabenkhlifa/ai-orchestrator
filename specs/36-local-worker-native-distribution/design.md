...
- Choice: Use a small native macOS shell (Swift/AppKit) whose only jobs are process lifecycle, the menu-bar UI, the URL-scheme handoff, and the update flow; it launches and supervises the existing worker runtime rather than reimplementing any worker behavior. That runtime ships as a new, separate `:worker` `mix release` target — no `releases:` key or `rel/` overlay exists in this repository today, so this slice creates one — whose boot path starts only `SddOrchestrator.Worker.Supervisor` and its own direct dependencies, and explicitly excludes `SddOrchestratorWeb.Endpoint`, `SddOrchestrator.Repo`, and every control-plane-only module from that release's applications.
- Reason: `specs/33-local-worker-run-execution`'s worker runtime is already implemented and `Verified`; re-deriving pairing, gateway, or run-execution logic natively would duplicate an already-proven contract and risk drift between two implementations of the same behavior. Excluding the control-plane code from the shipped release is a least-privilege boundary: an operator's laptop has no legitimate reason to hold a compiled copy of the hosted control plane's web routes, database schemas, or GitHub App integration logic, distinct from — and in addition to — the credential-custody boundary `specs/33-local-worker-run-execution` already established.
