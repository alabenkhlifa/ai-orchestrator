...
- The URL-scheme pairing payload carries only the single-use pairing code `specs/02-local-project-onboarding`'s pairing contract already defines, plus the project identifier the dashboard already knows when it renders the "Open in App" action. Activation is local OS inter-process communication, not a network transmission.
