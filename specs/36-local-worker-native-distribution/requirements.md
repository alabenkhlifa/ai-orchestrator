...
- [AC-06] Given the dashboard's pairing-code screen, when the operator clicks "Open in App," then the installed app is activated through its registered URL scheme carrying the current single-use pairing code and the project's identifier, and a machine with no installed app shows install guidance instead of a silent failure.
