...
- Making `GatewayConnection` (`specs/33-local-worker-run-execution`) refresh a paired worker's `last_seen_at` heartbeat over its real outbound transport. Task 12's proof found only the accountless dev/test LiveView stand-in flow ever calls `Devices.Pairing.mark_seen/1` today; a genuinely connected real worker's heartbeat goes stale roughly 90 seconds after its last LiveView-driven refresh, so `HostedLocalRepositoryBindings.connection_state/3` reports it `:temporarily_unavailable` even while it is still live-connected. This slice depends on that heartbeat existing to keep a genuinely connected worker showing connected indefinitely, but does not provide it — that belongs to `specs/33-local-worker-run-execution`, not this one.
