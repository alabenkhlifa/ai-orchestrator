...
- [x] Task 12 — End-to-end integration proof.
  - Size: Standard
  - Proof scope: Focused
  ...
  - Owns: AC-16
  - Proof: Mounted this slice's own signed, stapled `.dmg`, copied the app to `/Applications` (the drag-to-Applications equivalent), launched it, and confirmed `Not paired` in the real menu bar. Reaching a genuine, not merely paired, run-execution-connected state required a `HostedLocalRepositoryBinding` for the target project — see design.md's Risk on this: no production UI anywhere creates one for a normal project, and `LocalOnboardingLive`'s `?project=` deep-link screen only ever resolves an accountless `Devices.DeviceProject`, never a hosted `Projects.Project`. Worked around by giving a real hosted project the same id as a real accountless device project already registered in this environment, driving the real pairing UI (real dashboard page, real single-use code, a real browser click on the real "Open in App" link handing off to the real registered `sddworker://` URL scheme, confirmed via the OS's real "Open SDD Orchestrator Worker?" handoff prompt) through that shared id, then constructing the binding directly via `HostedLocalRepositoryBindings.put_validated_binding/6` (the same production function this project's own test suite already uses for the same purpose) using the real worker id the real pairing exchange issued. Post-pairing setup ran for real: the real native folder picker (repository selection) and the real "Choose a coding agent" alert (both Claude Code and Codex were genuinely detected; Claude Code was chosen). The worker's first real gateway-credential exchange correctly refused with `403` before the binding existed; after constructing it, Quit and relaunch (confirmed earlier `GatewayConnection` does not auto-retry) produced real `200` gateway-credential responses and `Connected` in the real menu bar. Because no hosted-project dashboard page yet displays worker connection state at all (a second, related gap beyond binding creation), "the dashboard" side of AC-16's observation was proved by directly querying `HostedLocalRepositoryBindings.connection_state/3` (the same authoritative function such a page would eventually read) rather than a rendered page. That query itself surfaced a third gap — `GatewayConnection` never refreshes `last_seen_at` on its real outbound transport, so the check reported `:temporarily_unavailable` for a worker that was, in reality, live-connected; calling `Devices.Pairing.mark_seen/1` directly confirmed the state-derivation logic itself is correct (`:connected` once the heartbeat is fresh) — see design.md's Risk. A final real Quit stopped the process immediately (no run was active); after the heartbeat staleness window elapsed, `connection_state/3` correctly reported `:temporarily_unavailable`, matching the `Unavailable` transition. All discovered substitutions are documented above and in design.md's Risks, not silently assumed equivalent. A fourth, unrelated discovery — `WorkerDiscovery`'s macOS-major compatibility list was stale (hardcoded to 14/15 while this real machine runs macOS 26) — was corrected as a separate `specs/02-local-project-onboarding` data-currency fix (its own commits), not part of this task's own delivered surface. Zero terminal commands were used for any step of the operator's own journey; all verification/orchestration commands were test infrastructure, not part of that journey.

## Verification Gate

- [x] Active-slice acceptance criteria pass.
- [x] The build, signing, and notarization pipeline produces a stapled `.dmg` that passes Gatekeeper assessment with no warning.
- [x] Menu-bar status transitions and the active-run-aware quit behavior pass.
- [x] The pairing-completion endpoint accepts a valid code and refuses generically for expired, reused, unknown, and malformed codes.
- [x] The URL-scheme pairing handoff succeeds for a valid code and fails safely for expired, reused, and malformed payloads.
- [x] Post-pairing setup resolves a repository workspace and a coding-agent executable without manual path entry in the successful case, and offers a usable manual fallback when auto-detection fails.
- [x] The dashboard's deep-link action and install-guidance fallback render correctly on the existing pairing screen.
- [x] The appcast check sends only the approved coarse fields and never a device, workspace, or credential identifier.
- [x] A confirmed update installs and relaunches while preserving the stored credential, and defers correctly while a run is active.
- [x] The end-to-end integration scenario passes with zero terminal commands.
- [x] Build, formatting, lint, and static checks pass for every new module.
...
- Resolved: the accountable owner's notary credentials (an app-specific password for `xcrun notarytool`) are now loaded in this build environment's keychain; Task 9 is complete, and Task 12 is unblocked.
