...
- Artifact-upload interface: accept one worker-initiated authenticated upload for an exact run and current fence, resolving the run's current attempt server-side rather than trusting a caller-declared attempt identity — the same proof `Event-ingestion interface` already applies; enforce the declared content type, size limit, and digest before storage; hand verified bytes to the authoritative artifact store; answer a duplicate upload of the same digest idempotently; and deny an unauthorized worker, another project, a superseded attempt, or a device-authoritative project without disclosing whether the content exists.
...
- Correction (found implementing `specs/33-local-worker-run-execution` Task 10): the transport originally required the caller to declare the target `RunAttempt`'s real database identity and matched it exactly, but no part of the worker protocol — the command envelope, event envelope, heartbeat, or reconciliation snapshot — ever transmits that identity to a worker; every one of them names an attempt only by `(run_id, attempt_number)` and proves currency through the fence token. This surface was implemented and proved (Task 52) but never exercised end to end against a real worker before Slice 33, so the gap was never caught. The corrected mechanism scopes an upload by run and current fence alone, exactly like `Event-ingestion interface` already does, which supplies the identical "this is the current, non-superseded attempt" proof: the fence token is minted per attempt and strictly increases on supersession, so a stale attempt's declared fence can never match the run's current one. No consumer-observable behavior changes — every refusal this transport already made (unauthorized worker, another project, a superseded attempt, a device-authoritative project) collapses to the same response before and after, since `WorkerArtifactController` already answers all of them identically.
