...
- Should `ModelCatalogs.current_catalog/3` and `Quotas.current_quota/3` also re-check connection eligibility on read, rather than the fix being presentation only? Recommended: no. `current_catalog/3` has exactly one caller inside `lib/`, this screen, so changing it is reachable but buys nothing the presentation fix does not already give. `current_quota/3` has two further callers: `QuotaPolicy`, where the effect is inert because it already refuses a revoking connection earlier, and `RuntimeProjections.owner_projection/3`, where a run pinned to a connection whose revocation was just requested would silently drop from real account-wide quota facts to `unknown`. That is a behavior change to approved AC-13 behavior in a `Verified` slice and would require an `update-spec` on `specs/11-ai-runtime-governance`, which this specification has no authority to make. Blocks: technical design, then Tasks 1 and 2.
