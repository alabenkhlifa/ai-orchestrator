...
### 2026-08-14 - Verification gate: fix pulled in, one accepted exception recorded

- Merged latest `main` (includes specs/25's fix for `Acceptance.profile_failure/1` misclassifying invalid/taken display names as `:identity_lifecycle_conflict`, discovered while first running this slice's own `mix test` gate). Re-ran the full suite: `test/sdd_orchestrator/participation/acceptance_test.exs` and `test/sdd_orchestrator_web/live/invitation_acceptance_live_test.exs` now pass cleanly.
- Accepted exception (same basis as specs/25, same two tests, same evidence — see `specs/25-participation-identity-lifecycle/progress.md` 2026-08-13 entry): `SddOrchestrator.Delivery.LocalWorkerRuntimeProjectionTest` and `SddOrchestrator.Delivery.RevocationConsumerTest` fail even in isolation, are confined to `lib/sdd_orchestrator/delivery/` files this specification never touches, and are unrelated to any of Tasks 1-5. A third and fourth failure in this run (`Worker.IsolationTest`, `StagingBuilderTest`) are pure test-pollution under full concurrent load — pass clean in isolation, not accepted as exceptions because they are not real failures.
- Failed checks: `mix test` — 3943/3947 passed, 4 failed; 2 confirmed genuine pre-existing/unrelated (accepted), 2 confirmed pollution (not real).
- Format, compile --warnings-as-errors, credo --strict, dialyzer, deps.audit, and sobelow --config all pass with zero issues (run before the merge; unaffected by it since no lib file changed).
- Remaining: Production proof (`MIX_ENV=prod mix assets.deploy` and `mix release --overwrite`), specification and capability-graph validators, then mark Verified and merge.
