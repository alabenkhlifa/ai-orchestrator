...
- The project assistant consumes `capability:project-specification-store` for current specification reads and `capability:guided-delivery-data-surfaces` only through read-only board, run, and evidence queries. It cannot append a specification revision or invoke a delivery command.
