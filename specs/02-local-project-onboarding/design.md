...
- Shared pairing guidance rendered by every surface that asks for a pairing code.
...
- Connection-status interface: distinguish connected, unavailable, authorization-required, and invalid states, deriving reachability from control-plane-observed worker attachment rather than from a worker-initiated liveness call.
...
### Computed macOS Compatibility Window

- Choice: Compute `WorkerDiscovery.compatibility_policy/0`'s supported macOS majors from a maintained `{major, GA release date}` reference table evaluated against the current instant, instead of a hardcoded current-plus-previous pair. The current major is the highest tabulated major whose release date has passed; the window is that major and the one immediately before it. A worker reporting exactly one major above the highest tabulated entry is still treated as compatible — forward tolerance for a released major not yet added to the table; a worker reporting two or more majors above the highest tabulated entry, or below the computed floor, remains incompatible.
- Reason: A hardcoded current-plus-previous pair drifts stale the moment a new macOS major ships and the constant is not edited, misclassifying a genuinely current, compatible worker as incompatible — the exact failure `specs/36-local-worker-native-distribution` Task 12 hit running the real signed worker on a real current-OS machine. Computing the window from elapsed time against a maintained release-date table, with one major of forward tolerance, keeps a genuinely current worker classified as compatible even before its table row is added.
- Consequence: The release-date table still needs one new row per Apple major — a single well-known public date, materially lighter upkeep than moving both bounds of a raw pair. The forward-tolerated major is accepted before its actual protocol and packaging compatibility is proven on that OS; this residual risk is bounded to exactly one major ahead and tracked as a Risk below.

### One Pairing Guidance, Rendered Everywhere

- Choice: Hold the pairing guidance as one value owned by this specification, in `Devices`, and render it on every surface that asks for a pairing code: accountless local onboarding, empty-repository initialization, and the hosted project page's connect action. The guidance carries the headline and the two steps that obtain the code, which are identical everywhere: open the worker app from the macOS menu bar, then click its status line to copy the code. Installation is a branch inside the first step, for someone who does not have the app, not the premise of the guidance. A surface appends its own paste step only when it actually offers a field. `Portability.HostedLocalRepositoryMachines.guidance/0` delegates to this value instead of holding its own wording, and the supported macOS window inside it is rendered from `WorkerDiscovery`'s computed policy rather than restated.
- Reason: The same two sentences had been written three times and drifted from the product they describe. `specs/38-worker-initiated-pairing` made the worker app obtain and show its own code, so the code now exists in the menu bar before anyone opens the dashboard, yet every guidance surface still described the app as absent and the code as something seen once on first launch. The control plane runs in a browser and cannot detect an installed application, so guidance that opens by asserting the app is missing is wrong for the common case and cannot be made right by detection. One owned value is the only shape in which a correction reaches all three surfaces at once.
- Consequence: `specs/16-empty-repository-initialization` and `specs/37-hosted-local-repository-connection` render text this specification owns. Their own criteria stay satisfied because they require graphical guidance with no terminal command, which the shared value provides; neither may reintroduce a local copy. The hosted project page shows the two obtain-the-code steps and no paste step, because it has no pairing field, which is recorded as separate work in that specification rather than papered over with an instruction the page cannot honor.
...
### Registry-Derived Worker Liveness

- Choice: Refresh `LocalWorker.last_seen_at` from the control plane's own record of live worker attachments rather than from any worker-initiated liveness call. A supervised periodic refresher enumerates the attached-worker registry `Delivery.CommandTransport.Channel` already maintains and marks every attached worker seen through the existing `Pairing.mark_seen/1`, on an interval shorter than the reachability staleness window. It runs in dev and production and is disabled in test, matching the established `Privacy.RetentionPruner` shape that tests drive directly instead of on a timer.
- Reason: Only the accountless dev/test LiveView stand-in ever called `Pairing.mark_seen/1`. A genuinely connected real worker therefore went stale about ninety seconds after the last stand-in refresh, so connection state reported temporarily unavailable while the worker was live — found by `specs/36-local-worker-native-distribution` Task 12 running the real signed worker on a real machine. A worker-initiated liveness call cannot close this: the failing case is a connected but idle worker, which emits no run heartbeat at all, and the worker may not reach `Devices.Pairing` directly because it is a separate process on the user's machine. A registry entry exists only while an authenticated channel process for that worker is alive, so the registry is a truthful liveness source that needs no protocol change, no worker change, and no new outbound data.
- Consequence: Liveness accuracy is bounded by the refresh interval rather than being instantaneous, which the staleness window already tolerates. Each control-plane node refreshes the workers attached to it, which stays correct if the control plane is ever run on more than one node. No new personal data is introduced: an existing timestamp on an existing authorization record is updated from a connection the user explicitly established.
...
- Corrected by the Computed macOS Compatibility Window decision: the window is now derived from a maintained release-date table plus one major of forward tolerance instead of a raw hardcoded pair, closing the staleness failure `specs/36-local-worker-native-distribution` Task 12 hit on a real current-OS machine. Residual risk: the forward-tolerated major is accepted as compatible before its actual protocol and packaging behavior is proven on that OS, and the release-date table still needs one new row added when Apple ships a major — lighter upkeep than the pair it replaces, but still a manual step tied to each macOS release.
- A paired worker's `last_seen_at` was refreshed only by the accountless dev/test LiveView stand-in, so a genuinely connected worker was reported temporarily unavailable once that refresh went stale. Closed by the Registry-Derived Worker Liveness decision above. Residual risk: liveness is only as fresh as the refresh interval, and a worker whose channel process is alive but whose runtime is wedged still reads as connected — the same bound any transport-level liveness signal carries.
