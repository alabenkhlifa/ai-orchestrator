...
- Hosted local-worker binding interface: authorize the hosted project owner and selected device workspace independently, select one active worker already paired to that device workspace, pass only the project ID and project-held portable identifier to the worker validation request, insert or atomically replace the minimum binding after exact success, support explicit disconnect, derive unavailable or revoked results without accepting package credentials, paths, or source connection data, and report a stored repository identity that does not parse as an error rather than as a connection state.
...
### Unparseable Repository Identity Is An Error, Not A State

- Choice: When a hosted local-repository project's stored canonical identity does not parse — a legacy workspace-scoped value, or a malformed one — the connection-state boundary returns `:invalid_repository_identity` rather than reporting `:disconnected`. Both `HostedLocalRepositoryBindings.connection_state/3` and the `HostedLocalRepositoryReconnection` wrapper that forwards it declare that error explicitly.
- Reason: `:disconnected` is a real, recoverable state that means "no machine is linked yet, link one". An identity no worker can prove is not that: no binding can ever exist for it, so answering with a state would invite a connect attempt that must always fail and would make an unusable project indistinguishable from a ready one. The two cases need different answers because they have different remedies — link a machine, versus upgrade the identity first.
- Consequence: Every caller must handle the error, not just the three states. A presentation surface that must still show one of the three states for such a project — as `specs/37-hosted-local-repository-connection`'s project page does under its own `AC-06` — decides that presentation on its own side from the project it already holds, and does not read this boundary's error as though it were a state. This decision confirms the behaviour that already shipped; it corrected two understated `@spec` declarations and changed no clause, branch, or returned value.
