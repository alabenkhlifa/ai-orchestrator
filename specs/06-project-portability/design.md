...
- Technical design and active-slice implementation blocker: how should an explicitly reconnected local repository be authorized and persisted for a restored hosted project? The existing `LocalWorker` credential authorizes one accountless `DeviceWorkspace`, the existing hosted `RepositoryConnection` is GitHub-specific and stores a numeric provider repository ID, and no approved personal-workspace-to-device-workspace association exists. Before Task 21 resumes, decide whether the approved hosted restore workflow may create an explicit, revocable hosted-to-device worker binding and define its authoritative record, minimum fields, uniqueness, access, retention, erasure and rights propagation, worker replacement and revocation behavior, and unavailable-device semantics without accepting a packaged path, credential, connection record, or source workspace identity.
