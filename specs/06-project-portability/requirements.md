...
14. When the restored local-repository project is hosted, the user explicitly authorizes one currently available device workspace and its active paired worker; only an exact match creates the minimum revocable hosted-to-worker binding.
...
- An explicit, minimal, revocable hosted-project-to-device-worker binding after exact local repository validation.
...
- Reconnecting a device-authoritative local project uses only that device workspace's normal worker authorization and persists no hosted project-to-worker binding.
- Reconnecting a hosted local-repository project requires the current action to hold the project's `PersonalWorkspace` authority and explicitly select an available `DeviceWorkspace` whose active paired worker authorizes the validation. Authentication to either boundary does not imply authority over the other, and the product must not create an account-to-device association silently or through package control.
- An exact hosted match creates one `HostedLocalRepositoryBinding` containing only the hosted project ID, the selected worker ID, and the last successful validation time. Project ownership is derived from the project, device-workspace authority is derived from the worker, and the canonical repository identity remains on the project; the binding must not duplicate an account or workspace ID, repository identifier, path, credential, connection payload, device label, or compatibility metadata.
- A hosted project has at most one local-worker binding. Repeating the same successful binding is idempotent. Replacing its worker commits only after the replacement worker proves the same project-held canonical repository identity; mismatch, malformed or legacy identity, unavailability, or failed authorization preserves the existing binding unchanged.
- Explicit disconnect, project erasure, worker revocation, or service termination deletes the hosted binding. Temporary worker unavailability is a derived presentation state and neither deletes nor rewrites the binding. A replacement worker must be explicitly authorized and exactly revalidated before it replaces the old worker reference.
...
- Package data, restored project data, hosted local-worker bindings, provenance, logs, and encrypted or otherwise linkable identifiers remain personal or confidential data; encryption does not make them anonymous.
...
- `HostedLocalRepositoryBinding` is processed only to route the user's explicit local repository connection for the hosted project under the approved service-performance basis. Access is limited to the owning `PersonalWorkspace`, the explicitly selected authorized worker for validation and repository operations, and approved operations personnel when necessary for security, support, lifecycle enforcement, or verified rights handling.
- `HostedLocalRepositoryBinding` is retained only while the project remains explicitly connected to that worker. Explicit disconnect, worker revocation, replacement, project erasure, and service termination remove the obsolete link; encrypted backup copies follow the approved 35-day expiry and deletion-propagation process.
...
- Verified access, correction, erasure, restriction, objection, and portability workflows cover service-held package attempts, hosted local-worker bindings, provenance, restored records, logs where applicable, processors, and backup-expiry handling. A self-service rights screen is not required in this slice.
...
- [AC-22] Given explicit local-repository reconnection is attempted for a device-authoritative restored project through its normal worker validation flow, when the selected repository is evaluated against the packaged portable canonical identifier, then only an exact recomputed match connects; a mismatch, malformed or legacy identifier, unavailable worker, or failed authorization leaves the project disconnected, and repository files, branches, remotes, settings, and Git configuration remain unchanged.
...
- [AC-27] Given the owning user explicitly reconnects a restored hosted local-repository project through a currently authorized device workspace and active paired worker, when that worker proves the exact project-held portable canonical identifier, then one minimal hosted local-worker binding is created or idempotently retained; disconnect or worker revocation removes it, replacement commits only after exact revalidation, and every failed or unavailable outcome leaves the previous binding and repository unchanged.
