...
- [AC-05] Given a compatible package, successful passphrase-based decryption, authorized destination storage, and no conflict, when restoration commits, then exactly one project is created atomically in the selected mode with the packaged stable project identity.
...
- [AC-10] Given explicit repository reconnection succeeds, when repository state is inspected, then repository files, branches, remotes, settings, and Git configuration remain unchanged.
...
- [AC-13] Given a user starts backup or restoration, when the interface presents package scope, validation, authority, conflict, or completion state, then it identifies the three included logical content categories, states that history, runs, artifacts, comments, attachments, logs, and repository source are excluded, and provides an actionable result without implying cross-user sharing or create-copy behavior.
...
- [AC-16] Given package generation or restoration reaches success, cancellation, failure, abandonment, project erasure, or service termination, when lifecycle enforcement runs, then the service retains no completed package; passphrases, derived keys, and decrypted temporary content are discarded immediately after use; terminal encrypted temporary data and attempts are deleted immediately and stranded copies within 24 hours; persistent provenance contains only schema version and restoration time and is deleted with the project; security logs contain only approved fields and expire within 30 days; encrypted rolling backups expire within 35 days; applicable processor and rights actions propagate; and no package data is reused for analytics, advertising, model training, unrelated improvement, or identity tracking.
