...
### The Mac connection retries the credential exchange itself, a project connection still does not

- Choice: `GatewayConnection` already retries forever after a drop that follows a successful join: `handle_disconnect/2` calls Slipstream's own `reconnect/1`, which reuses the held gateway credential and re-tries the transport with a backoff list that repeats its last value once exhausted, and that loop runs inside this same process without ever stopping it. The gap is earlier: `handle_continue(:connect_gateway, socket)` calls `establish/2`, whose `fetch_gateway_credential/1` is a plain `Req.post` outside Slipstream's own machinery entirely, and on any failure `handle_continue` answers `{:stop, :normal, socket}` unconditionally. `fetch_gateway_credential/1` already tells the two failures apart in its return value — `{:gateway_credential_refused, status, body}` for an HTTP response the control plane actually sent, `{:gateway_credential_transport_error, reason}` for one it never got to send. The fix is to let the Mac scope alone treat only the second as retriable: write `ConnectionStatus.set_disconnected/1`, schedule `handle_continue(:connect_gateway, socket)` again after a widening, repeating delay, and never stop the process for it. A refusal, and a malformed `:invalid_websocket_configuration`, keep the existing unconditional stop for both scopes.
- Reason: The two failures mean different things. A refused credential will not start working by asking again, and retrying it forever would be the exact mistake `specs/40-worker-repository-selection/`'s own moduledoc already names for a related case ("a request that cannot be published is a request the person will not see... crashing the worker over it would be worse" applies in reverse here: retrying a refusal forever spams the control plane over something retrying can never fix). An unreachable control plane is the opposite: nothing about the worker is wrong, and the credential it already holds is still good the moment the control plane comes back.
- Consequence: No supervisor restart policy changes, and `ProjectConnections`' own temporary children are untouched, because the fix keeps the Mac-scoped process alive and retrying internally rather than relying on OTP to bring a stopped one back. A worker now survives a control plane that goes away for any length of time, at the cost of a retry loop that must widen its delay and never log at a level that fills a log while it runs. A refusal keeps its existing single-answer path on both scopes and is still never retried.

### The report belongs to a live process

- Choice: The reported connection state is written by a process that is still trying, and a state nobody is maintaining does not read as a connection.
- Reason: The stale claim was the expensive half of this defect. The connection process stopped, so it wrote nothing further, and the last thing it had written was `connected`. The menu bar and the status file then asserted a connection for hours after there was none.
- Consequence: Truthfulness no longer depends on a stop path remembering to write. What a person reads is maintained while the worker is trying, and the dashboards keep deriving reachability from the control plane's own attachment record.
...
- Retrying forever could hide a control plane the person has genuinely moved away from, or fill a log. Reduced by widening the delay and by reporting the disconnected state the whole time; detected by the app's own state never claiming a connection it does not have.
