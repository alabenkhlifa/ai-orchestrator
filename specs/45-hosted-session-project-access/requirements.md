...
- [AC-01] Given a person signed in only through the passwordless email link owns a hosted project, when they open that project's address, then its dashboard renders with the repository, the storage mode, and the connection state.
