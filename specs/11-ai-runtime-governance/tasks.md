...
Tasks 7, 8, 1, 9, 2, 3, 10, 4, 13, 16, and 11 are complete: the personal-worker AI RPC transport, version-checked Codex App Server adapter, account-owned personal AI connection foundation, account-level AI Connections workflow, live model catalog with compatible effort validation, live quota and token-activity normalization, explicit quota and paid-use policy, immutable provider-neutral runtime-session pinning, personal-connection cleanup with credential-revocation reconciliation, model-catalog and quota-snapshot expiry, and strict API-key cost reservation are delivered on `slice/11-ai-runtime-governance`. Tasks 12, 14, and 5 are complete: ordered runtime observations are ingested with structural fact, estimate, and unknown labelling, runtime-session and cost-ledger retention, rights, and connection-removal behavior are enforced, and owner-exact and participant-safe runtime-observation projections are published. Tasks 4 and 13, then Tasks 16 and 11, then Tasks 12 and 14, were delivered in parallel under the recorded intra-slice ownership partitions below. `capability:ai-runtime-session` and `capability:ai-runtime-observation` are ready. Task 17 is now the next executable task, and repository-wide verification remains serialized until the parallel Slice 14 work is reconciled. One slice-gate blocker found during that parallel work — a cross-table false positive in the Slice 09 specification-governance table assertion, introduced by Task 2's migration — has been corrected and committed separately, as recorded in the progress log. Remote and cloud worker setup, project-shared funding, consumer-owned per-agent presentation, and Slice 07 lifecycle integration are deferred to focused follow-up specifications.
