...
Tasks 7, 8, 1, 9, 2, 3, 10, 4, 13, 16, and 11 are complete: the personal-worker AI RPC transport, version-checked Codex App Server adapter, account-owned personal AI connection foundation, account-level AI Connections workflow, live model catalog with compatible effort validation, live quota and token-activity normalization, explicit quota and paid-use policy, immutable provider-neutral runtime-session pinning, personal-connection cleanup with credential-revocation reconciliation, model-catalog and quota-snapshot expiry, and strict API-key cost reservation are delivered on `slice/11-ai-runtime-governance`. Tasks 12, 14, 5, 17, and 15 are complete: ordered runtime observations are ingested with structural fact, estimate, and unknown labelling, runtime-session and cost-ledger retention, rights, and connection-removal behavior are enforced, owner-exact and participant-safe runtime-observation projections are published, the observation trail expires on its own shorter operational window with full rights handling, and runtime operations are now held inside the approved service and security purposes by a complete processing inventory and field-purpose map, a fail-closed data-use policy, and a content-free operational log with structural redaction. Tasks 4 and 13, then Tasks 16 and 11, then Tasks 12 and 14, were delivered in parallel under the recorded intra-slice ownership partitions below. `capability:ai-runtime-session` and `capability:ai-runtime-observation` are ready. Task 6, the consolidated privacy and security review, is now the next and last executable task, and its prerequisite Task 15 is complete. Slice 14 has since merged into the default branch, and this slice branch has been rebased onto it; repository-wide verification still runs at the slice gate and has not yet run for this slice. One slice-gate blocker found during that parallel work — a cross-table false positive in the Slice 09 specification-governance table assertion, introduced by Task 2's migration — has been corrected and committed separately, as recorded in the progress log. Remote and cloud worker setup, project-shared funding, consumer-owned per-agent presentation, and Slice 07 lifecycle integration are deferred to focused follow-up specifications.
...
- Implementation is additionally partitioned by ownership between `Task 11` and `Task 16`, which share one working tree and are separated by disjoint paths, distinct test partitions, and distinct build paths.
- `Task 11` exclusively owns `RuntimeCostLedger`, its cost-reservation boundary module, its versioned official-price snapshot registry, its migration, its focused test, and `test/support/ai_runtime_fixtures.ex`. It consumes `RuntimeSessions.get_session/2` and `RuntimeSessions.fetch_for_consumer/3` as a read-only caller and modifies no runtime-session, personal-connection, catalog, quota, or privacy surface.
- `Task 16` exclusively owns `ModelCatalogs`, `Quotas`, the configured catalog and quota lifetimes, the `ModelCatalogSnapshot` and `QuotaSnapshot` entries in `Privacy.Retention` and `Privacy.Rights`, and its focused test. It does not modify `test/support/ai_runtime_fixtures.ex`, whose catalog and quota snapshot fixtures already accept `now:` and `ttl_seconds:`, and it does not touch any cost-ledger or runtime-session surface.
- Neither of these two tasks modifies `application.ex` or `config/config.exs`. Both read configuration through `Application.get_env/3` with fail-closed module defaults, following the established snapshot-lifetime and Codex compatibility-registry pattern, so the deployment's official price source stays in the release gate.
- Implementation is additionally partitioned by ownership between `Task 12` and `Task 14`, which share one working tree and are separated by disjoint paths, distinct test partitions, and distinct build paths.
- `Task 12` exclusively owns `AgentRuntimeObservation`, its ordered-append boundary module and migration, the observation adapter with its RPC implementation and deterministic double, its focused test, and `test/support/ai_runtime_fixtures.ex`. It modifies no runtime-session, cost-ledger, catalog, quota, or privacy surface, and it delivers no projection: owner-exact and participant-safe views remain owned by `Task 5`.
- `Task 14` exclusively owns the `AIRuntimeSession` and `RuntimeCostLedger` entries in `Privacy.Retention` and `Privacy.Rights`, the session and ledger retention and connection-removal behavior, and the existing privacy test files. It consumes the established `runtime_session_context_fixture/1`, `ai_runtime_session_fixture/2`, and `runtime_cost_ledger_fixture/2` as a read-only caller and does not modify `test/support/ai_runtime_fixtures.ex`.
- `Task 14` owns the confirm-or-replace decision on the provisional `AIRuntimeSession.connection_id` foreign-key behavior that `Task 4` recorded as downstream-owned. `Task 12` must not change it.
...
- [x] Task 11 — Enforce strict API-key cost reservations and publish runtime-session readiness.
  - Size: Standard
  - Depends on: Task 4
  ...
  - Delivered: `RuntimeCostLedger` and its migration persist one strict ceiling row per API-key runtime session, holding currency, ceiling, versioned official-price evidence, bounded request configuration, outstanding reservations, reconciled observed cost, and pause state, with no provider invoice or payment credential. `RuntimeCosts` locks the account, then the session row, then the ledger row `FOR UPDATE` inside one transaction, refuses a ChatGPT session as `not_applicable`, computes the conservative maximum only in `Decimal` rounded up at scale 4 so it can never under-reserve, and returns a typed resumable pause that preserves the pinned configuration when remaining capacity cannot cover the next bounded turn. Outstanding reservations are one `jsonb` map keyed by idempotency key rather than a second entity: an identical replay returns the stored reservation, the same key with different bounds fails as `duplicate_reservation`, reconciliation refuses an observed cost above its own reservation, and abandoned reservations are released rather than charged after a configurable timeout. Over-allocation is defended by the row lock, by an application invariant tying `reserved_amount` to the decoded reservation sum, and by a database `CHECK (reserved_amount >= 0 and observed_amount >= 0 and reserved_amount + observed_amount <= ceiling)` proven under deliberate lock bypass. `RuntimeCosts.PriceSnapshot` defaults to an empty registry so an unconfigured deployment refuses every reservation, revalidates the whole registry on each lookup, and fails closed as `missing_price` or `stale_price`; the deployment's real price source remains release-gated. Runtime-session, quota, catalog, and connection surfaces are consumed read-only and unchanged. Session and ledger retention and rights remain owned by Task 14.

- [x] Task 12 — Ingest ordered runtime observations.
  - Size: Standard
  - Depends on: Task 11
  ...
  - Delivered: `AgentRuntimeObservation` and its migration persist one ordered minimized observation per agent under a composite `(account_id, session_id)` foreign key to `ai_runtime_sessions(account_id, id)` with `on delete cascade`, so an observation always follows its session and can never hold a mismatched account. `RuntimeObservations` appends under a session row lock, enforces ordering through a unique `(session_id, sequence)` index proven by direct insert, replays an identical event idempotently through a unique `(session_id, event_key)` index, and refuses a conflicting duplicate, an out-of-order or superseded event, an observation predating the pin, and one running ahead of the local clock. Source labelling is enforced by database check constraints rather than convention: each of the five value groups carries one label from `provider_fact`, `worker_observed`, `local_estimate`, or `unknown`; `cost_source` admits only `local_estimate` or `unknown`, so an estimate cannot be stored as a provider invoice; `unknown` is equivalent to an absent value and every absent value must name itself. An estimate stores and cross-checks its basis against the reported counters and the session's pinned model. `ObservationAdapter` validates an exact bounded result with size limits, credential-shaped-content rejection, and refusal of any worker-profile echo, and its Codex implementation ingests the already-admitted thread token-usage and rate-limit notifications through the existing verified App Server boundary. A paused status must name a resumable reason. Ingest against a session whose connection was removed fails closed while its stored history stays readable. Projections, owner and participant authorization, and consumer presentation remain owned by Task 5, Slice 07, and Slice 12.

- [x] Task 5 — Publish access-safe runtime-observation projections.
  - Size: Standard
  - Depends on: Task 12
  ...
  - Delivered: `RuntimeProjections` publishes `capability:ai-runtime-observation` as a read-only boundary that composes the pinned-session, ordered-observation, quota, and cost-ledger boundaries and writes nothing. `owner_projection/3` authorizes by account ownership alone, so another account's session is refused as `not_found` rather than disclosed, and it carries the account-wide quota, its credit and paid-continuation facts, the run's strict ceiling, and the owner's estimated cost. `participant_projection/4` re-asks `Participation.Boundary` on every call for current membership of the named project plus `read_run_evidence`, resolves the session by its own reference and re-reads every projected fact through `RuntimeSessions.get_session/2` inside its owning account so Task 4 keeps sole authority over session facts, and refuses a support-assistant session because a personal support conversation is not a project run. It carries only the selected model, effort, project-run elapsed time and token counters, and a safe availability state; account-wide quota, credits, spend, cost, provenance, opt-ins, consumer reference, connection reference, and provider identity never cross it, proven by a recursive key walker with its own planted-key negative control rather than by a shallow key check. Absent evidence is always an explicit marker: an unknown or expired quota and an unopened API-key ledger project `state: :unknown`, a ChatGPT session's ceiling projects `state: :not_applicable`, and unobserved usage keeps its shape with `source: :unknown`, so nothing absent can read as zero or unlimited. Every participant denial is one identical `unavailable`, so a denied caller learns neither whether the project or session exists nor which check failed. Slice 11 records no project on a session and none was added; binding a run reference to its project remains the calling slice's obligation, as the retirement boundary already requires.
...
- [x] Task 16 — Enforce model-catalog and quota-snapshot expiry.
  - Size: Standard
  ...
  - Delivered: `Privacy.Retention.prune_ai_runtime_snapshots/1` deletes every catalog and quota snapshot whose stored short lifetime has passed, and every snapshot belonging to a connection that is terminally revoked or already scheduled for deletion, under its own PostgreSQL session advisory lock taken inside `Repo.checkout/1`. The key is deliberately distinct from the revocation sweep's, so a contended revocation pass cannot silently suppress snapshot expiry; contention yields `:locked` and deletes nothing. The sweep is registered last in the already-supervised `prune_all/1`, so a connection that becomes terminal earlier in the same pass loses its evidence in that pass and the counts never double-count rows the connection cascade already removed. `Privacy.Rights` now exports both snapshot entities explicitly with their stored expiry instead of omitting them as a cache, and `terminate_personal_ai_service/1` deletes catalog and quota evidence in the same account-scoped or global scope it already revokes, closing a gap where that evidence survived termination until the next retention pass. `ModelCatalogs` and `Quotas` needed no behavioral change: the `expires_at > now` currency boundary, the 300-second default, the 3600-second maximum, and the fail-closed refusal of expired evidence at projection, selection, and session pinning were confirmed exact and are now recorded in their moduledocs. No migration was required because both tables already carry an `expires_at` index and a cascading `connection_id` foreign key.

- [x] Task 14 — Enforce runtime-session and cost-ledger lifecycle and rights controls.
  - Size: Standard
  ...
  - Delivered: `ai_runtime_sessions.connection_id` is now nullable with `on delete set null`, replacing Task 4's provisional cascade, and the immutability trigger permits exactly one additional transition — a present connection reference becoming absent — while still refusing null to non-null and any change of connection. Connection removal therefore destroys the opaque reference, which is what must go, and leaves the pinned configuration as accountability evidence carrying no credential, provider identity, worker-profile reference, or raw provider error. `runtime_cost_ledgers.session_id` keeps its cascade, so a ledger never outlives its session and now survives connection removal with it. `Privacy.Retention` prunes an attached session and its ledger 90 days after the pin and a detached one after 30 days, running last in `prune_all/1` under its own advisory lock and yielding `locked` under contention; pause is never a deletion trigger, so a paused session stays resumable for its whole window. `Privacy.Rights` exports sessions and ledgers explicitly against an allowlist proven equal to the schema fields minus account and update metadata, erases them with the account, retains them through personal-AI service termination under an explicit accountability disposition, refuses correction of the immutable pinned configuration with the database itself proving the refusal, returns verified-operator assessment for restriction and objection, and adds `retire_runtime_consumers/2` as the account-scoped idempotent deletion handoff that project erasure now calls. `AIRuntimeSession` records no terminal lifecycle marker and none was added, because Slice 07 lifecycle integration is deferred; run and conversation deletion must call the retirement boundary with their own references when that integration lands.

- [x] Task 17 — Enforce runtime-observation lifecycle and rights controls.
  - Size: Standard
  ...
  - Delivered: `Privacy.Retention.prune_ai_runtime_observations/1` deletes every observation 30 days after it was observed, under its own advisory lock key distinct from the pruner, revocation, snapshot, and runtime-session keys; contention yields `locked` and deletes nothing. The window is deliberately shorter than the session's 90-day attached and 30-day detached windows, because the trail serves the near-term purpose of operating and pausing work safely, which is the purpose and lifetime the deployment already fixes for its operational security log, while the account of what the run executed under and what it cost is held by the session and its ledger and outlives the trail. The sweep is registered last in `prune_all/1`, after the session sweep, so a session deleted in the same pass has already cascaded its observations away and the reported count never double-counts them. Losing the trail is a real loss of access and is projected honestly: an expired run keeps its stable shape and reports `source: unknown` for elapsed time, tokens, estimated cost, and availability for both the owner and a current participant, so nothing absent can read as zero. `Privacy.Rights` exports observations against an allowlist proven equal to the schema fields minus account and update metadata, with the estimate basis and quota references decoded so the access copy is readable and every value's label travels beside it. Correction is refused as `refused_immutable_operational_record`, and the refusal is structural rather than policy: the entity exposes only a create changeset, `(session_id, sequence)` and `(session_id, event_key)` are both unique, and restating one event key with different facts is refused at ingest as `duplicate_event`. Restriction and objection return the verified operator assessment with its processor, derived-copy, and encrypted-backup handoff. `retire_runtime_consumers/2` now deletes and reports observations alongside the session and ledger, so a project or conversation deletion can say what it removed rather than leaving it to the cascade, and `terminate_personal_ai_service/1` counts the trail it retains, because an observation describes work that already ran and revoking the connection cannot undo it. No migration, schema, or fixture change was required.

- [x] Task 15 — Enforce minimized runtime operations and prohibited-use controls.
  - Size: Standard
  ...
  - Delivered: `Privacy.ProcessingInventory` now records the seven AI-runtime processing activities — the personal connection reference, the model catalog and quota projections, the pinned session, its cost ledger, the observation trail, and the operational log — each with its purpose, lawful basis, minimized field list, least-privilege access boundary, real retention window, rights behavior, processors, and transfers, and `ai_runtime_field_purposes/0` gives every persisted field of all six entities a recorded purpose whose key set is proven equal to the schema's own fields, so a field added without a purpose fails the proof. The six record activities are `contract` because each is necessary to deliver the requested runtime; the operational log alone is `legitimate_interests` for security and reliability. `Privacy.AIRuntimeDataUsePolicy` is the fail-closed purpose-and-recipient boundary: it refuses advertising, analytics, identity tracking, model training, and unrelated product improvement for every data class, refuses an advertising network, analytics processor, coding agent, and model provider as a recipient, and expresses least privilege in the route map itself — `approved_operations` reaches only `retention_cleanup`, `verified_rights_operator` only `verified_rights`, and a current project participant only the pinned session and observations, never account-wide allowance, credits, or spend. `AIRuntime.SecurityLog` emits one structured line of exactly four allowlisted keys, and only for a failure, so the log is an audit trail rather than a running record of what each account does; its closed outcome vocabulary admits only allowlisted reason atoms and its final clause maps anything else to `failed` without inspecting or encoding it, which is the redaction mechanism rather than a filter. It is wired at the single `PersonalWorkerRPC` request boundary, where every catalog, quota, connection, and observation operation crosses to the worker, returning its argument unchanged so no behavior moved. Expiry is the deployment's own 30-day operational-log ceiling and the encrypted 35-day backup lifecycle, both read from `DeploymentPrivacyProfile` rather than restated. The database negative scan proves no AI-runtime column, index, or table name carries credential or analytics content, with its eleven documented counter and lifecycle exceptions each asserted still present in the schema and bounded to fixed vocabularies. No migration, schema, or fixture change was required.
