...
An owner-reviewed repository execution profile gains one authoritative pilot and independent readiness, then becomes a privacy-governed deterministic managed-runtime capability without copying source, specification documents, credentials, or repository files into a new authority.
...
- Privacy, security, retention, deletion, rights, processor, transfer, logging, analytics, and secondary-use controls for the completed assessment, worker-local proposal payload, proposal envelope, worker cache, profile, pilot, and readiness records.
- Owner selection of one current authoritative project specification revision as the pilot without copying documents or importing repository backlog.
- Separate assistant, specification, agent-execution, and release readiness with actionable earliest-stage blockers and a reliable required-check gate.
...
7. After its focused tasks and complete deterministic verification gate pass, it publishes `capability:repository-execution-profile` for separately approved consumers.
...
- Assessment, proposal-envelope, cache, profile, pilot, readiness, and compatibility data are prohibited from product analytics, advertising, model training, and unrelated reuse.
