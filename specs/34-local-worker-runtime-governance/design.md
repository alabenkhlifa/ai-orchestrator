...
- The owner-exact projection is readable only by the run's current initiator, the account that owns the spent connection; the participant-safe projection is readable only by a current authorized project participant (which includes the project owner when they are not the initiator), exactly as `specs/11-ai-runtime-governance` already enforces with no project-owner exception to account ownership.
