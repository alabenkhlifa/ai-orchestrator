...
- A local-worker observation source that derives elapsed time and status from the worker's own attempt lifecycle (workspace ready, progress heartbeat, verification completed, terminal state) without contacting the Codex App Server RPC, and that always reports token and cost facts as unknown.
- Ingesting one ordered `capability:ai-runtime-observation` at each of those lifecycle points for a governed run, and applying the connection's most recently retrieved quota snapshot to the projection as informational context only.
- Showing the pinned connection label, model, effort, and latest observation to the run initiator and project owner (owner-exact) and a safe project-run projection to other current authorized participants, reusing `specs/11-ai-runtime-governance`'s existing projection contracts.
- GDPR purpose, minimization, access, retention, deletion, and rights coverage for the new run-to-session link and its observation records.
...
6. As the run's attempt reaches workspace-ready, a progress heartbeat, verification-completed, and its terminal state, the product ingests one ordered observation from the run's own lifecycle facts for a governed run only.
7. The run initiator and project owner see the pinned connection, model, effort, and latest observation next to the run's activity. Other current authorized participants see only the safe project-run view. An ungoverned run shows no runtime projection.
...
- A governed run's observations report only elapsed time and status derived from the worker's own attempt lifecycle, plus the connection's most recently retrieved quota snapshot shown as informational context. Token counts and cost are always reported unknown for this slice; neither is estimated, guessed, or inferred from agent output.
...
- No repository content, absolute path, agent transcript, or provider credential may appear in a connection-selection request, a pinned session's consumer reference, or an ingested observation.
...
- [AC-05] Given a governed run's attempt reaches workspace-ready, a progress heartbeat, verification-completed, or a terminal state, when an observation is produced, then it reports elapsed time and status derived only from the worker's own attempt lifecycle, the connection's most recently retrieved quota snapshot as informational context, and token and cost values as explicitly unknown.
- [AC-06] Given a governed run has one or more ingested observations, when the run initiator or project owner requests its runtime state, then they see the pinned connection label, model, effort, and latest observation; when another current authorized participant requests it, they see only the safe project-run view; an ungoverned run shows no runtime projection to anyone.
- [AC-07] Given a run's connection-selection request, pinned session consumer reference, and ingested observations are inspected, when privacy and security verification runs, then none contains repository content, an absolute path, an agent transcript, or a provider credential, and existing `specs/11-ai-runtime-governance` retention, deletion, and rights controls cover every new record.
- [AC-08] Given a governed local-worker run and an ungoverned local-worker run both execute end to end on a real local repository, when the slice's verification gate runs, then the governed run's session, ingested observations, and projections are all real and correct, the ungoverned run behaves identically to the unmodified `specs/33-local-worker-run-execution` baseline, and `capability:local-worker-runtime-governance` publishes only after both prove out.
