...
- The log review is split rather than blanket. LiveView renders `save_requirements` parameters and `create_feature`'s title at `:debug`, and Ecto logs the same values as query parameters. Production runs at `:info` and emits neither, which the review asserts. Development set no level and printed them on the developer's own machine. First recorded as known and accepted; the user then chose to close it, and `config/dev.exs` now sets `:info` to match production. The cost is accepted: Ecto query logging and every other debug line go with it.
