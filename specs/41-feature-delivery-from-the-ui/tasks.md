...
- Product proof for AC-07 and AC-08 is now partly closed against the paired Mac worker, on 2026-09-04. Every start precondition was met by clicking, `Start development` was pressed, one run was created on `sdd/run-f122c9fe-b944-49cb-a286-1e269a328587`, the feature moved to `In development`, and the worker acknowledged the start command. `run_commands` recorded `start` as `acknowledged`, and the worker's own `run_state.json` recorded the attempt as `accepted` with its branch, fence token, and manifest digest. That is the acknowledgement half of AC-07 against a real worker rather than a double.
- The previous blocker is gone. This gate used to say that completing an assessment against a real Mac needed a live worker-backed metadata adapter. `specs/47-live-repository-metadata-binding/` delivered that adapter and `specs/46-assessing-a-repository-on-a-mac/` delivered the live scan, both `Verified` by their own click paths. An approved execution profile is now reachable by clicking, and it was approved on this project before the press.
- What blocks the gate now is different, and was not previously recorded. The run stalls immediately after acceptance: `run_attempts` stays `pending` with `last_sequence` 0 and no progress ever reaches the screen. The cause is confirmed in code, not inferred. `ExecutionPreparer.prepare/2` calls `Branch.prepare/1`, which calls `Workspace.prepare/1`, which reads `Workspace.root/0`; that answers `{:error, :workspace_root_unconfigured}` whenever `:worker_workspace_root` is unset, and `Worker.Supervisor` leaves it unset for a worker whose configuration names no project. A Mac-scoped worker paired through `specs/38-worker-initiated-pairing/` has `workspace_root` and `project_id` both absent, so it has no folder in which to prepare a run workspace. The assessment path never meets this, because it resolves the repository through the folder the worker is already holding for the binding rather than through a configured root.
- A second fault sits beside it and makes the first one invisible. `GatewayConnection.prepare_execution/3` logs a preparation refusal and returns the socket unchanged, sending nothing to the control plane. The run therefore stays `pending` with no reason recorded anywhere the product can read, and the feature screen keeps saying `Development is running. Progress appears here.` indefinitely. AC-08 requires a refused start to show its reason; a start that is accepted and then silently fails to prepare is outside what AC-08 describes and is not covered by it.
- Both belong to one unwritten specification, not to this slice. It owns how a Mac-scoped worker resolves the repository folder for a run on a hosted project whose repository it is already bound to, and how a preparation failure after acceptance reaches the person. This slice does not decide either, and stays not releasable until that specification is delivered and this gate's remaining proof runs: press `Start development` and see the worker's first progress from this Mac.
