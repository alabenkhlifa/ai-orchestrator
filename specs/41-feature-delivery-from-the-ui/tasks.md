...
- That blocker is closed as of 2026-09-03. `specs/44-hosted-local-repository-projects/` delivers creating a hosted local-repository project from the storage step, and `specs/45-hosted-session-project-access/` lets the person who signs in through the email link reach it. Both are `Verified` by one click path from `/` in a real browser against the paired worker app, and that project's feature board was confirmed reachable on the same session. That click path was then driven as far as it goes on 2026-09-03. Four of the five start preconditions are met by clicking. The fifth needs an approved execution profile, and `specs/46-assessing-a-repository-on-a-mac/` has since removed the gates that refused to assess such a repository. What remains is not a defect: completing an assessment against a real Mac needs a live worker-backed metadata adapter, which `specs/14-repository-execution-profile/` deliberately deferred to its own release gate, `Live configured worker smoke proof for each supported deployment profile`. Until that gate closes, the assessment is refused for every project including a GitHub one, so AC-07 and AC-08 stay proven only by their domain and integration tests and this slice is not releasable.
