...
- `Start.start/4` builds the manifest from the project's approved `RepositoryExecutionProfile`: `repository_base_revision` from `base_revision`, `required_checks` from `required_checks`, and the profile's `commands`, `allowed_scope`, and `root` in the manifest's agent and worker references. It refuses with `:no_execution_profile` when none is approved. The `:delivery_execution` application key is removed; the browser-suite bootstrap seeds an approved profile for its delivery scenarios instead of setting the key. The worker binding check is a precondition, so a run is never started for a project whose worker is not attached now.
- When a hosted project becomes bound to a worker, and again whenever that worker attaches for its Mac while bindings exist, the control plane pushes `project_bound` with the project id over the Mac-scoped attachment. The worker exchanges its credential for a project-scoped gateway credential through the existing exchange `specs/33-local-worker-run-execution/` defines, joins the project's `worker:` topic on its existing socket, and from then on receives that project's run commands through the unchanged `deliver/1`. Disconnecting the project pushes `project_unbound` and the worker leaves the topic.
...
- `Delivery.Start` (`preconditions/3`, `available?/3`, `start/4`) and `ExecutionManifest`: profile-derived manifest, `:no_execution_profile`, worker-attached precondition.
- `RepositoryAssessments` approved-profile lookup for a project.
- Mac-scoped attachment channel from `specs/39` and `specs/40`: `project_bound` and `project_unbound` messages; `Worker.GatewayConnection`: project credential exchange and topic join on demand.
...
### The worker joins a project's run topic when told

- Choice: The control plane pushes `project_bound` over the Mac-scoped attachment, and the worker joins the project's `worker:` topic on demand using the existing project credential exchange.
...
- Consequence: One more message pair on the Mac attachment, and a worker that serves several projects holds several topic joins on one socket. The Mac attachment remains the only place the worker learns about bindings.
...
- A worker that serves a project it was bound to before this slice, through a restore, already joins the project topic from its configuration. `project_bound` for such a worker must be idempotent.
