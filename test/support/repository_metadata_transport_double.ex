defmodule SddOrchestrator.RepositoryMetadataTransportDouble do
  @moduledoc """
  A deterministic repository-metadata transport for tests.
  ...
  Unlike `SddOrchestrator.CommandTransportDouble`, the recording cannot live
  in the calling process dictionary: the transport is called by
  `SddOrchestrator.RepositoryMetadata.Server`, which is a different process
  from the test, and from whatever process is blocked inside `inspect/2`. It
  lives in an `Agent` started by `install/1` and reached through application
  environment, so the test and the server read and write the same recording.
  Tests using this double are `async: false` for the same reason the double
  exists: it swaps application environment.
  ...
  @state_key :repository_metadata_transport_double
  ...
  @spec pushed() :: [SddOrchestrator.RepositoryMetadata.MetadataRequest.t()]
  def pushed, do: :pushed |> read([]) |> Enum.reverse()

  @doc "The requests the transport was told to cancel, oldest first."
  @spec cancelled() :: [SddOrchestrator.RepositoryMetadata.MetadataRequest.t()]
