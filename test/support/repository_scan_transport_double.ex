defmodule SddOrchestrator.RepositoryScanTransportDouble do
  @moduledoc """
  A deterministic repository-scan transport for tests.

  Records what the request server handed it and replays a scripted push
  outcome, so the request lifecycle (one outcome per blocked call, expiry,
  the worker's channel going down, the requester's exit, and refusal of a
  foreign answer) can be proven without an attached worker.

  This is a second copy of `SddOrchestrator.RepositoryMetadataTransportDouble`
  rather than a shared one, for the same reason the two contexts are separate:
  the doubles stand in for different transports whose vocabularies must not be
  widened together by one shared helper.

  The recording cannot live in the calling process dictionary: the transport
  is called by `SddOrchestrator.RepositoryScan.Server`, which is a different
  process from the test, and from whatever process is blocked inside `run/2`.
  It lives in an `Agent` started by `install/1` and reached through
  application environment. Tests using this double are `async: false` for the
  same reason the double exists: it swaps application environment.
  ...
  current, and a test kills it to prove the `:worker_unavailable` path that
  comes from losing the worker's channel.
  """
  @behaviour SddOrchestrator.RepositoryScan.Transport

  @transport_key :repository_scan_transport
  @state_key :repository_scan_transport_double
  ...
  @spec pushed() :: [SddOrchestrator.RepositoryScan.ScanRequest.t()]
  def pushed, do: :pushed |> read([]) |> Enum.reverse()

  @doc "The requests the transport was told to cancel, oldest first."
  @spec cancelled() :: [SddOrchestrator.RepositoryScan.ScanRequest.t()]
  ...
  the attachment resolves the blocked call to `:worker_unavailable`.
  """
  ...
  # finishes handling a requester that exited. Recording is then dropped
