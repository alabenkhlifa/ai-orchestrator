...
14. Copy all four bundled templates from `assets/` into `specs/<feature>/` and replace every placeholder. `progress.md` holds the `### ...` progress entries newest first, and `## Progress Log` in `tasks.md` stays reduced to the single pointer line `See [progress.md](progress.md).` A new specification starts with no entries; the file must still exist so the implementation workflow has one place to record proof receipts, failed checks, and environment incidents.
